...
{
    int rc = 0;
    if (board != NULL && conf != NULL) {
        if (cc_hashtable_size(board) != 0 || cc_hashtable_size(conf) != 0) {
            return -1;
        }
        cc_hashtable_destroy(board);
        board = NULL;
        cc_hashtable_destroy(conf);
        conf = NULL;        
    }
    if (hMutex != NULL) {
        CloseHandle(hMutex);
...
int trig_mode(const char *ip, int internal)
{
    WaitForSingleObject(hMutex,INFINITE);
    ...
    CC_ARRAY_FOREACH(channel, chs, {
        Channel *ch = (Channel*)channel;
        ch->trig_mode = internal;
        // printf("trig_mode %d\n", ch->trig_mode);
    });
    Channel *ch_ctrl;
    cc_array_get_at(chs, 0, &ch_ctrl);
    int rc = board_trig_mode(ch_ctrl);
    if (rc < 0) {
...
int read(int key, void *data, int data_size, int channel_size)
{
    ...
    channels_buffer_realloc(arr_chs, copy_size*3);
    // channels_readcount_rst(arr_chs);
    // show_board(key);
    // printf("reading\n");
...
{ 
    ...
    // uint8_t *data = (uint8_t*)calloc(sizeof(uint8_t), copy_size);
    Channel *ch;
    ...
    while(1) {
        ...
        CC_ARRAY_FOREACH(channel, chs, {
            ...
            // if (ch->id == -1) {
            //     printf("ch -1 buf:%s\n", ch->buffer);
            //     if (strstr(ch->buffer, "OK") == NULL || strstr(ch->buffer, "sample_enable") == NULL){
            //         return -6;
            //     } 
            // }
            break;
...
int channels_read_finish(CC_Array *chs, int copy_size)
{
    ...
    CC_ARRAY_FOREACH(channel, chs, {
        ...
        if (ch->ctrl == 0) {
            finish++;
        } else if (ch->read_count >= copy_size) {
...
int channels_buffer_realloc(CC_Array *chs, int copy_size)
{
    Channel *channel;
    CC_ARRAY_FOREACH(channel, chs, {
        Channel *ch = (Channel*)channel;
...
int hash_config(const char* str) {
    int hash = 0;
    int len = strlen(str);
...
int parse_config(const char* config, CC_Array *arr_ip) 
{
    ...
    while (line != NULL) {
        ...
        if (sscanf(token, "ai%d", &id) != 1) {
            if (sscanf(token, "trig?internal=%d", &internal) == 1) {
                line = strtok_s(NULL, "\n", &context1);
                continue;
            } else {
                id = -2;
...
int board_trig_mode(Channel *ch_ctrl)
{
    char *msg = NULL;
    if (ch_ctrl->trig_mode == EXTERNAL) {
        msg = ">trig_mode(1,0)\r\n";
    } else if (ch_ctrl->trig_mode == INTERNAL) {
        // external
        msg = ">trig_mode(0,1)\r\n";
    } else {
...
int show_board(const int key)
{
    ...
    CC_ARRAY_FOREACH(ip, arr_ip, {
        ...
        CC_ARRAY_FOREACH(channel, channels, {
            ...
            if (ch->trig_mode == EXTERNAL) {
                printf("trig_mode: EXTERNAL  ");
            } else {
                printf("trig_mode: INTERNAL  ");
            }
...
int test1(uint32_t *data, int size)
{
    for (int i = 0; i < size; i++) {
        ...
        int tmp = (int)(TO_LITTLE_ENDIAN(*ptr) + 1);
        uint32_t val = (float)(tmp)/0x7fffff * 4.096 * 2;
        *ptr = val;
    }
    return 0;
}
